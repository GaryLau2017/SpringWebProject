...
<html>
    ...
    <body>
        ...
        <c:choose>
            ...
            <c:otherwise>
                <c:forEach items="${postDatabase}" var="entry4">
                    <c:if test="${entry4.categories == 'other'}" >
                        Post #${entry4.id}:
                        <a href="<c:url value="/post/view/${entry4.id}" />"> 
                    ...
                    <security:authorize access="hasRole('ADMIN') or principal.username=='${entry4.customerName}'">            
                        [<a href="<c:url value="/post/edit/${entry4.id}" />">Edit</a>]
                    </security:authorize>
                        </security:authorize>
                    <security:authorize access="hasRole('ADMIN')">            
                        [<a href="<c:url value="/post/delete/${entry4.id}" />">Delete</a>]
                    </security:authorize>
                    <hr>
                    </c:if>  
                </c:forEach>
